        ...
        if   c == '(':
            ...
            while True:
                ...
                if string[i].isdigit():
                    ...
                    break
                i += 1
            tokens.append(Token(TokenType.NUMBER, value=int(''.join(digits))))
        i += 1
    return tokens
...
def generate_test_expression():
    ...
    expr = [str(random.randrange(0, 50))]
    paren_level = 0
    for i in range(expr_len):
        ...
        if random.randrange(0, 100) < 35:
            ...
            paren_level += 1
            opened = True
        expr.append(str(random.randrange(1, 50)))
...
def test_alg():
    for i in range(10000):
        ...
        if py_value != my_value:
            print("Error with expression '{0}':\n  py_value: {1}\n  my_value: {2}".format(expr, py_value, my_value))
            exit()
